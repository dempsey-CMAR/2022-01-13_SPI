...
- This is the first time we've shared these results, so they are "hot off the press" and may change a little as we get feedback from folks like yourselves, but I definitely think it's worth sharing at this stage. 

**
- I'll start with discussing the motivation for the paper, and then talk about the model we used before jumping into the analysis and results.
...
- There has been a recent trend in the industry to stock post-smolts that are closer to 500 g or even a full 1 kg. There are proposed land-based facilities in NS that could grow post-smolts up to 1.5 kg
...
- There are 5 different depths, represented by colour
...
???
- The first thing I did was tell the code to look through all the temperature data from all of the locations we just saw and assign growing seasons, i.e., a date when fish could theoretically be put into the water, and when they would be removed.
- Based on advice provided by Roland, the season starts when the temperature exceeds the 4-degree threshold, which is marked by the dashed grey line in the figure, and doesn't go back down below it, 
...
- Obviously it is much easier to define the seasons this way in retrospect  

- did this for all stations + depths
...
min1 <- min(short_filt$TIMESTAMP) - days(n_days)
max1 <- max(short_filt$TIMESTAMP) + days(n_days)

short_raw <- dat_raw %>% 
  filter(STATION == "Madeline Point",
         TIMESTAMP >= min1, TIMESTAMP <= max1)

p1 <- plot_filtered_data(short_raw, short_filt,
...
min2 <- min(med_filt$TIMESTAMP) - days(n_days)
max2 <- max(med_filt$TIMESTAMP) + days(n_days)
...
min3 <- min(long_filt$TIMESTAMP) - days(n_days)
max3 <- max(long_filt$TIMESTAMP) + days(n_days)

long_raw <- dat_raw %>% 
  filter(STATION == "Flat Island",
         TIMESTAMP >= min3, TIMESTAMP <= max3)

p3 <- plot_filtered_data(long_raw, long_filt,
                         date_breaks_major = "2 month",
                         date_labels_format = "%Y-%b", 
                         plotly_friendly = FALSE) +
  plot_theme
...
???
- The next step was to calculate the number of growing days at each station, for the degree-day calculation
- We assumed that there would be no growth for 24 hours after the temperature exceeded 18 degrees Celcius, as illustrated in this figure.

- This is data from Madeline Point, our example of a short growing season. All of the observations greater than 18 degrees C, in the red part of the figure AND 24 hours after were filtered out of the analysis and not included in the average temperature OR number of degree days

- 31 days filtered out for 2 m depth; only 3.5 filtered out at 22 m
...
???
- depends on the TGC value assumed and in some cases the depth
- if we take a look at Madeline Point, our shortest season, assuming a fast-growing stock, a 1.5 kg smolt could grow out to market size in just over 8 months.

- For Beaver Point, 0.9 - 1.5 kg smolts could grow out to market size in the 9 month season
...
- Operators want 6+ years of data 
]
.pull-right[
**Assumptions**
...
- We have nearly 4 years of data for some sites, so one thing I'm excited to start doing is to explore this variability to get a better sense of timing of season start and superchill, and spatial correlations.
- Operators that they want at least 6 years of data before they make decisions

Assumptions
- Modelling study
(e.g., fish stay at one depth for the season, heat stress)
- Perfect knowledge of the exact best time to put fish in and take them out
